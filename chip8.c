...
// Global logging flag (enabled by default)
...
void emulate(Chip8 *chip) {
  ...
  case 0x2000: // 2nnn -> CALL (Call subroutine at nnn)
  {
    ...
    CHIP8_LOG("Call subroutine at %d\n", nnn);
  } break;
  ...
  case 0x3000: // 3xkk -> SE (Skip next instruction if Vx = kk)
  {
    ...
    if (chip->V[x] == kk) {
      chip->pc += 2;
      CHIP8_LOG("SE skipping next instruction\n");
    }
  } break;
  ...
  case 0x4000: // 4xkk -> SNE (Skip next instruction if Vx != kk)
  {
    ...
    if (chip->V[x] != kk) {
      chip->pc += 2;
      CHIP8_LOG("SNE skipping next instruction\n");
    }
  } break;
  ...
  case 0x5000: // 5xy0 -> SE (Skip next instruction if Vx = Vy)
  {
    ...
    if (chip->V[x] == chip->V[y]) {
      chip->pc += 2;
      CHIP8_LOG("5xy0 Skipping next instruction (V[%d] != V[%d])\n", x, y);
    }
  } break;
  ...
  case 0x8000:
    ...
    case 0x4: // 8xy4 -> ADD (Set V[x] = V[x] + V[y], set V[F] = carry)
    {
      ...
      CHIP8_LOG("8xy4: V[%d] += V[%d] (Result: %d, Carry: %d)\n", x, y,
                chip->V[x], chip->V[0xF]);
    } break;
    ...
    case 0x5: // 8xy5 -> SUB (Set Vx = Vx - Vy, set VF = NOT borrow)
    {
      ...
      chip->V[x] -= chip->V[y];
      CHIP8_LOG("SUB: V[%d] = V[%d] - V[%d] (VF=%d)\n", x, x, y, chip->V[0xF]);
    } break;
    ...
    case 0x7: // 8xy7 -> SUBN (Set V[x] = V[y] - V[x], set V[F] = NOT borrow)
    {
      ...
      CHIP8_LOG("SUBN: V[%d] = V[%d] - V[%d] (VF=%d)\n", x, y, x, chip->V[0xF]);
    } break;
  ...
  case 0x9000: // 9xy0 -> SNE (Skip next instruction if Vx != Vy)
  {
    ...
    if (chip->V[x] != chip->V[y]) {
      chip->pc += 2;
      CHIP8_LOG("9xy0 Skipping next instruction (V[%d] != V[%d])\n", x, y);
    }
  } break;
  ...
  case 0xB000: // Bnnn -> JP (Jump to location nnn + V[0])
    ...
    CHIP8_LOG("JP to 0x%X + V[0](0x%X) = 0x%X\n", (opcode & 0x0FFF), chip->V[0],
              chip->pc);
    break;
  ...
  case 0xC000: // Cxkk -> RND (Set Vx = random byte AND kk)
  {
    ...
    CHIP8_LOG("RND V[%d] = %d (Rand AND %d)\n", x, chip->V[x], kk);
  } break;
  ...
  case 0xD000: // Dxyn -> DRW (Display n-byte sprite)
  {
    ...
    // CHIP8_LOG("DEBUG DRAW: I=%X | X=%d Y=%d | H=%d\n", chip->I, x, y,
    // height);
  ...
  case 0xE000:
    switch (opcode & 0x00FF) {
    case 0x9E: // Ex9E -> SKP (Skip next instruction if the key with value Vx is
               // pressed)
    {
      uint8_t x = (opcode & 0x0F00) >> 8;
      if (chip->keypad[chip->V[x]]) { // pressed = 1
        chip->pc += 2;
        CHIP8_LOG("Ex9E keypad at %d pressed\n", chip->V[x]);
      }
    } break;

    case 0xA1: // ExA1 -> SKNP (Skip next instruction if the key with value Vx
               // is not pressed)
    {
      uint8_t x = (opcode & 0x0F00) >> 8;
      if (!chip->keypad[chip->V[x]]) {
        chip->pc += 2;
        CHIP8_LOG("ExA1 keypad at %d not pressed\n", chip->V[x]);
      }
    } break;
  ...
  case 0xF000:
    ...
    case 0x07: // Fx07 -> LD (Set Vx = delay_timer)
    {
      ...
      CHIP8_LOG("Fx07 V[%d] = %d(delay_timer)\n", x, chip->delay_timer);
    } break;

    case 0x0A: // Fx0A -> LD Vx (Wait for a key press, store the value of the
               // key in Vx.)
    {
      uint8_t x = (opcode & 0x0F00) >> 8;
      ...
      if (!key_pressed) // Iterate over the same instruction
        ...
        CHIP8_LOG("Key %d pressed, stored in V[%d]\n", chip->V[x], x);
    } break;
    ...
    case 0x29: // Fx29 -> LD F (Set I to location of sprite for digit V[x])
    {
      ...
      chip->I = digit * 5;
      CHIP8_LOG("Set I to font char '%X' at address %X\n", digit, chip->I);
    } break;

    case 0x33: // Fx33 -> LD B (Store BCD representation of V[x] in memory
               // locations I, I+1, and I+2)
    {
      ...
      CHIP8_LOG("BCD of %d stored at I(%d)\n", value, chip->I);
    } break;
    ...
    {
      uint8_t x = (opcode & 0x0F00) >> 8;
      CHIP8_LOG("SET I(%d) = I(%d) + Vx(%d)\n", chip->I, chip->I, chip->V[x]);
      chip->I += chip->V[x];
    } break;

    case 0x65: // Fx65 -> LD V[x] (Fills V[0]...V[x] with values from memory
